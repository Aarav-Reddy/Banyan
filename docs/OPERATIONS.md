...
Restore verification observed on the native environment: 43 tables and 1,137 rows passed a full snapshot/hash comparison. This evidence is for that run and does not certify later backups. See `docs/BUILD_STATUS.md` for the latest exact command results, restart evidence, browser evidence and remaining release blockers.
