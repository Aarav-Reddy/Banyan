...
## Clean-run CI follow-up

GitHub run 35453409784 passed backend/component/build checks and thirteen browser cases but exposed an upload waiting behind the initial seed recommendation queue. The downloaded trace showed repeated successful GETs returning queued. Targeted card-review recommendation jobs now reauthorize only the changed card for each recipient instead of scanning every unrelated card repeatedly. The E2E fixture runs the real leased worker to finish seed events and checks their terminal states before timing interactive journeys. Separately, a timer-controlled component regression ensures queued → processing → needs-mapping keeps polling; no browser timeout or behavioral assertion was relaxed. A new focused policy/query-growth regression covers the targeted recommendation calculation.
